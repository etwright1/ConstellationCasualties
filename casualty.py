...
def create_constellation(incs, proportions, totalsats):
    '''inclinations in degrees, propotions in decimal'''
    satellite_inclinations = []

    for i in range(0,len(incs)):
        no_sats = np.int(proportions[i]*totalsats)
        for j in range(0,no_sats):

            satellite_inclinations.append(incs[i])
    np.savetxt('Constellation_inclinations.csv', satellite_inclinations)
    return satellite_inclinations